...
class MusicPlayer {
  constructor(
    data = {
      ...
      songObjArr: [
        {
          title: "夢を束ねて（捆绑梦想）", // 歌曲名
          author: "結束バンド", // 歌手
          imgSrc: "./public/images/miku1.png", // 图片路径
          url: "./public/audios/牧野由依_-_シンフォニー.flac", // 音频路径
        },
        {
          title: "独りのためのララバイ",
          author: "パール兄弟",
        ...
        {
          title: "Hell Is Other People他人即地狱",
          author: "Holy Mercy聖慈/Dehumanizing Itatrain Worship",
          imgSrc: "./public/images/03.jpg",
          url: "./public/audios/03.mp3",
        },
        {
    ...
    this._musicPlayerEntity.addComp(
      new MusicControlComp({
        ...
        onClickMPlayBtn: () => {
          if (
            this._startTime === null &&
            this._musicPlayerEntity.getComp("musicComp").isCanPlay
          ) {
            this._musicPlayerEntity.getComp("musicComp").isPlay =
              !this._musicPlayerEntity.getComp("musicComp").isPlay;
            this._isUserPlay =
              this._musicPlayerEntity.getComp("musicComp").isPlay;
          }
        ...
        onClickMProgressBar: (event) => {
          ...
          if (
            event.target ===
            this._musicPlayerEntity.getComp("musicControlComp").mProgressBtnDom
          ) {
            return;
          }
          const time = this._getXTime(event.offsetX);
          this._musicPlayerEntity.getComp("musicControlComp").currentTime =
            time;
        ...
        onMousedownMProgressBtn: (event) => {
          ...
          this._musicPlayerEntity.getComp(
            "musicControlComp"
          ...
          const onMImgBoxMousemove = (event) => {
            ...
            if (currentX < 0) {
              ...
              currentX >
              this._musicPlayerEntity.getComp("musicControlComp").totalProgressW
            ) {
              currentX =
                this._musicPlayerEntity.getComp(
                  "musicControlComp"
          ...
          // 操作完成回调
          const removeCB = () => {
            this._mImgBoxDom.removeEventListener(
              "mousemove",
              onMImgBoxMousemove
            );
    ...
    this._musicPlayerEntity.addComp(
      new MusicComp({
        ...
        onCanplay: () => {
          ...
          this._musicPlayerEntity.getComp("musicControlComp").totalTime =
            this._musicPlayerEntity.getComp("musicComp").totalTime;
        ...
        onTimeupdate: () => {
          if (
            this._musicPlayerEntity.getComp("musicControlComp").isSetProgressBar
          ) {
            return;
          }
          this._musicPlayerEntity.getComp("musicControlComp").currentTime =
            this._musicPlayerEntity.getComp("musicComp").currentTime;
          this._musicPlayerEntity.getComp("musicControlComp").loadTime =
            this._musicPlayerEntity.getComp("musicComp").loadTime;
        },
      })
    );
  }
  ...
  set _currentIndex(value) {
    if (this._startTime === null) {
      ...
      this._musicPlayerEntity.getComp("musicImgComp").switchDir =
        value - this._currentIndex;
      this._musicPlayerEntity.getComp("musicDescComp").isAnim = true;
      this._musicPlayerEntity.getComp("musicDescComp").switchDir =
        value - this._currentIndex;
  ...
  _getXTime(value) {
    return Math.floor(
      (value * this._musicPlayerEntity.getComp("musicComp").totalTime) /
        this._musicPlayerEntity.getComp("musicControlComp").totalProgressW
    );
  }
  ...
  _renderFrame(timestamp) {
    ...
    if (timestamp >= this._startTime + 1000) {
      ...
      this._musicPlayerEntity.getComp("musicImgComp").currentImgIndex =
        this._currentIndex;
    ...
    if (timestamp >= this._startTime + 500 && !this._isRunTimeArr[1]) {
      ...
      this._musicPlayerEntity.getComp("musicDescComp").currentDescIndex =
        this._currentIndex;

      // 切换音乐
      this._musicPlayerEntity.getComp("musicComp").currentUrlIndex =
        this._currentIndex;
    }
  }
}
...
class MusicImgComp {
  ...
  set switchDir(value) {
    this._mImgContentDom.style.transform = `translateY(${
      -100 * (this._currentImgIndex + value + 1)
    }%)`;
...
class MusicControlComp {
  constructor(data) {
    ...
    this.mProgressBtnDom.addEventListener(
      "mousedown",
      data.onMousedownMProgressBtn
    );
...
class MusicDescComp {
  constructor(data) {
    ...
    this._mDescArticleDom = document.querySelector(
  ...
  set switchDir(value) {
    this._mDescArticleDom.style.setProperty(
